...
**Give Users The Dexterity To Explore Hypotheses:** 

-   The first page of the Shiny app will identify popular locations using credit and loyalty data. The user should be able to drill into popular locations i) by weekdays vs weekends, ii) by category, iii) by amount, and iv) by hour. Users can flesh out behaviours that seem suspicious. These credit card and loyalty card numbers will be further investigated in the next page.
- The second page of the Shiny app seeks to identify suspicious behaviour by travel pattern. Users should be able to view travel patterns by ‘location to location’ (node to node) at different time of day. Users can compare travel patterns across different cars or zoom into an individual by filtering using the car id.
- The third page of the Shiny app seeks to identify employees by inferring owners to each credit card and loyalty card. The visual will be able to filter to by car id, employee name, credit card number and loyalty card. The chart will be done in a time-series with i) transactional data (credit card and loyalty card) as the y-axis, date/time as the bottom x-axis, and location as the top x-axis. The analysis should be done with statistical approach using confidence interval/ hypothesis testing.

**Sharing Insights from the Investigation:** 

- The last page of the Shiny app seeks to highlight insights gathered from the earlier three pages and flesh out i) unofficial relationships and ii) locations where suspicious activity may be occurring. The page will not be interactive. 
